  ...
  // GET /mensajes
    ...
    async meInteresa(req, res) {
        ...
        try {
            ...
            const titulo = publicacion ? publicacion.titulo : 'tu publicacion'

            // posicion de la imagen dentro de la publicacion para mostrarla en el mensaje
            const imagenes = await Publicacion.obtenerImagenes(imagen.publicacion_id)
            const posicion = imagenes.findIndex(img => img.id === parseInt(imagen.id)) + 1
            const totalImagenes = imagenes.length

            const textoImagen = totalImagenes > 1 ? `imagen ${posicion} de ${totalImagenes}` : `publicacion "${titulo}"`

            await Mensaje.crear({    
                remitente_id: usuario_id,
                destinatario_id: imagen.usuario_id,
                imagen_id: imagen_id,
                contenido: `Hola, me interesa adquirir la ${textoImagen}. ¿Podemos hablar?`,
            })
